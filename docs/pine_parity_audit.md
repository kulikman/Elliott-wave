...
## Итог: что нужно исправить в Python

| Приоритет | Файл | Изменение |
|-----------|------|-----------|
| 🔴 Высокий | `python/ewb/figures.py::_try_double_corr` | Добавить Y ≥ 61.8%W check |
| 🟡 Средний | `python/ewb/confirm.py::confirm_flat` | C ≥ 61.8%A → severity E (не N) |
| 🟡 Средний | `python/ewb/confirm.py::confirm_triangle` | Добавить e < c check при len≥6 |
| ⚪ Обсудить | `pine/ewb_monowaves_mtf.pine` r4 | Понизить W2 retrace до warning vs текущий mandatory |
| ⚪ Заблокировано | Zigzag | Ждём OCR Гл.3 |

## Примечание о Triangle severity
...
- Требует анализа impact через backtest — задача для отдельного спринта
